...
```sh
$ docker pull namely/protoc-all:1.13
```
...
$ docker run -v `pwd`:/defs namely/protoc-all:1.13 -f myproto.proto -l ruby #or go, csharp, etc
```

```powershell
PS> cd ~/my_project/protobufs
PS> docker run -v ${pwd}:/defs namely/protoc-all:1.13 -f myproto.proto -l ruby #or go, csharp, etc
```
...
```
$ docker run ... namely/protoc-all:1.13 -i protorepo -f catalog/catalog.proto -l go
# instead of
$ docker run ... namely/protoc-all:1.13 -f protorepo/catalog/catalog.proto -l go
